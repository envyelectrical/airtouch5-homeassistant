...
from homeassistant.helpers.entity_platform import AddConfigEntryEntitesCallback
...
async def async_setup_entry(
    ...
    async_add_entities: AddConfigEntryEntitesCallback,
) -> None:
...
class Airtouch5AC(Airtouch5ClimateEntity):
    ...
    def __init__(self, client: Airtouch5SimpleClient, ability: AcAbility) -> None:
        ...
        self._attr_unique_id = f"{self._config.entry_id}_{self._config.entry_id}_ac_{ability.ac_number}"
        self._attr_device_info = DeviceInfo(
            identifiers={(DOMAIN, f"{self._config.entry_id}_ac_{ability.ac_number}")},
...
class Airtouch5Zone(Airtouch5ClimateEntity):
    ...
    def __init__(
        ...
        self._attr_unique_id = f"{self._config.entry_id}_zone_{name.zone_number}"
        self._attr_device_info = DeviceInfo(
            identifiers={(DOMAIN, f"{self._config.entry_id}_zone_{name.zone_number}")},
            name=name.zone_name,
            manufacturer="Polyaire",
            model="AirTouch 5",
        )
